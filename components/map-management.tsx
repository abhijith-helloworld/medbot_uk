...
import {
    ...
    useActivateMap,
    useDeleteMap,
} from "@/features/admin/hooks/useMap";
...
if (isLoading) {
    ...
    return (
        <>
            <div className="mx-auto space-y-6 px-4 md:px-6 lg:px-10 py-4">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Activate or delete robot navigation maps from the
                        available list.
                    </p>
                </div>
                ...
                {activeMap && (
                    <div>
                        <h2 className="text-2xl font-semibold tracking-tight border-b pb-4 mb-6">
                            Active Map
                        </h2>
                        {/* UI REWORK: Banner-style card for active map, no image */}
                        <div className="p-4 bg-green-50 dark:bg-slate-900 border-l-4 border-green-500 rounded-lg flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <Map className="w-8 h-8 text-green-700 dark:text-green-700" />
                                <div>
                                    <h3 className="text-lg font-semibold dark:text-green-700 text-green-900">
                                        {activeMap.map_name}
                                    </h3>
                                    <p className="text-sm dark:text-green-700 text-green-800">
                                        Created:{" "}
                                        {new Date(
                                            activeMap.created_at
                                        ).toLocaleDateString()}
                                    </p>
                                </div>
                            </div>
                ...
                <div>
                    <h2 className="text-2xl font-semibold tracking-tight border-b pb-4 mb-6">
                        Available Maps ({inactiveMaps.length})
                    </h2>
                    {inactiveMaps.length > 0 ? (
                        ...
                        <div className="space-y-4">
                            {inactiveMaps.map((map) => (
                                ...
                                <Card
                                    key={map.id}
                                    className="p-4 flex items-center justify-between transition-shadow hover:shadow-md"
                                >
                                    <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex items-center gap-2 flex-shrink-0 ml-4">
                                        <Button
                                            ...
                                            size="sm"
                                            onClick={() =>
                                                handleSetActiveMap(map.id)
                                            }
                                            disabled={isActivating}
                                        >
                                            {isActivating &&
                                            activatingMapId === map.id ? (
                        ...
                        <div className="flex flex-col items-center justify-center gap-4 text-center py-16 border-2 border-dashed rounded-lg">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-muted-foreground">
                                    When new maps are added, they will appear
                                    here.
                                </p>
                            </div>
                        </div>
            ...
            <Dialog
                open={!!mapToDelete}
                onOpenChange={(isOpen) => !isOpen && setMapToDelete(null)}
            >
                <DialogContent>
                    <DialogHeader>
