...
interface Slot {
  id: number;
  is_active: boolean | string | number; // backend may return boolean / "true" / 1
  room_name: Room | null;
  bed_name: Bed;
  created_at: string;
  updated_at: string;
...
const normalizeIsActive = (val: boolean | string | number | undefined | null) => {
...
export function InfrastructureManagement() {
  ...
  // Derived State for Slots (use normalized is_active)
  ...
  const activeSlots = useMemo(
    () =>
      assignedSlots?.filter((slot) => normalizeIsActive(slot.is_active)) ||
      [],
    [assignedSlots]
  );
  ...
  const handleToggleSlotStatus = async (
    ...
    try {
      // NOTE: deleteSlot is used because your hook exposes that name.
      // If backend expects PATCH/PUT to toggle, update your hook implementation.
    ...
    <div className="mx-auto space-y-6 px-4 md:px-6 lg:px-10 py-4">
      <div className="">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
          {/* ... Your four stats cards are unchanged ... */}
              ...
              <Dialog open={isAddRoomOpen} onOpenChange={setAddRoomOpen}>
                ...
                <DialogContent className="sm:max-w-md">
                  ...
                  <div className="flex gap-3 py-4">
                    ...
                    />
                    <Button onClick={handleCreateRooms} disabled={roomCount < 1}>
                      Create
                    </Button>
                  </div>
                </DialogContent>
          ...
          <Card className="border-border">
            ...
            <CardContent className="pt-2">
              ...
              <Dialog open={isAddBedOpen} onOpenChange={setAddBedOpen}>
                <DialogTrigger asChild>
                  <Button variant="outline" size="sm" className="w-full">
                    <PlusCircle className="mr-2 h-4 w-4" />
                    Add Beds
                ...
                <DialogContent className="sm:max-w-md">
                  <DialogHeader>
                    <DialogTitle>Create New Beds</DialogTitle>
                    <DialogDescription>
                      Enter the number of beds you want to create.
                    </DialogDescription>
                  </DialogHeader>
                  <div className="flex gap-3 py-4">
                    <Input
                    ...
                    />
                    <Button onClick={handleCreateBeds} disabled={bedCount < 1}>
                      Create
                    </Button>
                  </div>
                </DialogContent>
          ...
          <Card className="border-border">
            ...
            <CardContent className="pt-2">
              <div className="text-2xl font-semibold text-foreground">
                {isLoadingSlots ? "..." : activeSlots?.length || 0}
              </div>
...
"
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Dialog open={isAssignSlotOpen} onOpenChange={setAssignSlotOpen}>
              <DialogTrigger asChild>
              ...
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  <DialogTitle>Assign a Bed to a Room</DialogTitle>
                  <DialogDescription>
                    Create a new slot by assigning a bed to a room.
                  </DialogDescription>
                </DialogHeader>
                <Form {...form}>
                  <form
                    onSubmit={form.handleSubmit(onSlotSubmit)}
                            ...
                            <SelectContent>
                              {beds?.map((bed) => (
                                <SelectItem
                                  key={bed.id}
                                  value={String(bed.id)}
                                  disabled={(bed as any).is_booked} // keep previous behavior
                                  className={(bed as any).is_booked ? "text-red-500" : ""}
                                >
                                  {bed.bed_name} {(bed as any).is_booked ? "(Booked)" : ""}
                                </SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
        ...
        <CardContent>
          {isLoadingSlots ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-foreground mb-2">No slots assigned</h3>
              <p className="text-muted-foreground mb-4">Start by assigning beds to rooms.</p>
              <Dialog open={isAssignSlotOpen} onOpenChange={setAssignSlotOpen}>
                <DialogTrigger asChild>
                  <Button>
                    <PlusCircle className="mr-2 h-4 w-4" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {assignedSlots.map((slot) => {
                const isActive = normalizeIsActive(slot.is_active);
                          ...
                          <div className="flex items-start gap-2 pt-1">
                            ...
                            <div className="flex flex-col">
                              {slot.x !== undefined && slot.y !== undefined ? (
                                <>
                                  <span>Position:</span>
                                  <span className="pl-2 font-mono text-xs">x: {Number(slot.x).toFixed(4)}</span>
                                  <span className="pl-2 font-mono text-xs">y: {Number(slot.y).toFixed(4)}</span>
                                </>
                              ) : (
                                <span>Position: Not set</span>
                              )}

                              {slot.yaw !== undefined ? (
                                <span className="mt-1">Angle: {Number(slot.yaw).toFixed(4)}</span>
                              ) : (
                                <span className="mt-1">Angle: Not set</span>
                              )}
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="mt-6 pt-4 border-t border-border">
                        <div className="grid grid-cols-2 gap-2">
                          <Button
                            ...
                            onClick={() => handleToggleSlotStatus(slot.id, isActive)}
                          >
        ...
        >
          <DialogContent className="sm:max-w-lg">
            <DialogHeader>
              <DialogTitle>
                Slot Details: {viewingSlot?.room_name?.room_name} / {viewingSlot?.bed_name.bed_name}
              </DialogTitle>
              <DialogDescription>Detailed information for the selected slot.</DialogDescription>
            </DialogHeader>
            ...
            {viewingSlot && (
              <div className="py-4 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <Badge variant={normalizeIsActive(viewingSlot.is_active) ? "default" : "secondary"}>
                    {normalizeIsActive(viewingSlot.is_active) ? "Active" : "Inactive"}
                  </Badge>
                </div>
                <Separator />
                <h4 className="font-semibold text-foreground">Position Data (Full Precision)</h4>
                <div className="space-y-2 text-sm font-mono bg-muted p-3 rounded-md">
                  <p>
                    X-coordinate: <span className="font-semibold">{viewingSlot.x ?? "N/A"}</span>
                  </p>
                  <p>
                    Y-coordinate: <span className="font-semibold">{viewingSlot.y ?? "N/A"}</span>
                  </p>
                  <p>
                    Angle (Yaw): <span className="font-semibold">{viewingSlot.yaw ?? "N/A"}</span>
                  </p>
                </div>
                <Separator />
                ...
                <div className="space-y-2 text-sm text-muted-foreground">
                  <p>
                    Slot ID: <span className="font-medium text-foreground">{viewingSlot.id}</span>
                  </p>
                  <p>
                    Created On: <span className="font-medium text-foreground">{formatDate(viewingSlot.created_at)}</span>
                  </p>
                  <p>
                    Last Updated: <span className="font-medium text-foreground">{formatDate(viewingSlot.updated_at)}</span>
                  </p>
                  <p>
                    Created By: <span className="font-medium text-foreground">{extractCreatorName(viewingSlot.created_by)}</span>
                  </p>
                </div>
              </div>
            )}
